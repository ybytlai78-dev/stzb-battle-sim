...
单将行动：被动 → 指挥预备判定（战必/措手/白衣 目标行动时）→ 二类指挥判定（奇兵拒北）→ 混乱检查 → 怯战检查 → 准备战法检查 → 主动战法（**准备槽按战法记录，可同时多段准备**：逐槽独立判定 —— 进入准备不跳过其余主动；释放回合只跳过被释放的那个战法）→ 普通攻击（连击至多 2 次，**每次命中后立即结算分兵**）→ 追击战法
回合结束：tickStatuses 状态递减/移除
```

**准备槽（`UnitState.preparations`，用户 2026-09-22 口径）**：每个准备主动各占一格 `{ skillId, left }`，同一武将可同时准备多个；
本将每次行动开始先结算准备槽 —— `left > 1` 则 −1 继续准备，`left === 1` 则 `prepare_end` 释放并从槽内移除；
随后逐槽判定主动战法，**准备中的战法与「本回合刚释放」的战法跳过判定，其余（瞬发 / 其他准备战法）照常判定**。
例：双准备 A/B → 第 1 回合 A 成功 / B 失败 → 第 2 回合释放 A 并继续判 B（B 成功后登记自己的槽）→ 第 3 回合释放 B。
...
**例外 —— 士气提升不与任何效果冲突（用户 2026-09-22）**：`morale_boost` **正值**一律独立共存（同源仍走刷新 / 显式叠层），
由 `effectiveMorale` 逐实例相加 —— 例如 司马炎【谋议宏图】每回合 +8 与 司马徽【徽言龙凤】+10 同时生效（不再取较高替换）；
士气**降低**（负值）仍按原规则（同号取较高、与提升正负相反各自共存）。
**不同类型** → 各自独立计数共存。
...
- **实现**：`src/engine/secondaryTroop.ts` `effectiveTroopLine(general)`；**五处调用点全部按兵系**（用户 2026-09-22 确认「三处都改」）：
  ① 战法段 `out.troopTypes` 目标过滤（衡轭「步兵普攻伤害 +50%」→ 蛮兵吃得到）；
  ② `teamPassesTroopFilter` 阵容门闩（「我军仅由骑兵和步兵组成时生效」）；
  ③ 兵种相克 `troopCounterReduceOf`（蛮兵祝融按步系：步克弓、被骑克；长枪兵对骑/对弓分支同样用目标兵系）；
  ④ 兵种加成 `troopBonus.ts` + 部队加成弹窗（`teamEditor.ts` 多数兵种名）按兵系计数；
  ⑤ 典藏条件「3 名武将兵种相同」`teamTroopsSame`（蛮兵/步兵/藤甲兵同属步兵系 → 相同）。
- 修正案例：华雄【衡轭】此前只给骑兵发谋略 +50（蛮兵祝融拿不到步兵段）；现蛮兵祝融吃**步兵段普攻增伤**、
  不再吃骑兵段谋略（兵系单一归属，不叠加两段）。测试 `tests/troop_line_conversion.test.ts`（9 个）。
...
- **2026-09-20 修正（田丰误登记下架）**：`jinyan_zhijian` 曾按「两段受谋略」误登记 OFFLINE → 复核官方 desc（200966 现页 +
  ...
  现存唯一另一处命中是 `xuefen_duanbing`（属**有意**登记：动摇 DoT 成长率未确认，非误判）。
- **用户 2026-09-20 二次口径（新增，勿再询问）**：
  ① **「大幅度」= 极大值**：凡官方写「大幅降低」而无数字的，一律 rate = **−99.99（−9999%）**（配合 buffMult 10%
     伤害下限压到最低；战报显示「大幅降低」）——已用于 方阵掩杀 / 迟智难酬 / 鏖兵卫主 / 锦车持节（造成侧）/ 酒池肉林；
  ② **自擅江表「对友军群体发动一次攻击」是官方原文、非笔误**（确为打友军 + 后续效果）；
  ③ **尽言直谏**：增益**持续到目标本次行动结束就消失**（非常规「下次行动前递减」）→ `expireAfterOwnAct`；
  ④ **酒池肉林**三段结构 = 前两回合减伤（持续到第 3 回合行动前）+ 第 3 回合我方造成伤害大幅降低（极大值）+ 第 4 回合起吸血；
  ⑤ **曹彰 五兵之烈 挂起**（等宝物系统上线，本轮不做）。
...
- **§1.4「需先调研」45 位进度**（总表 `docs/research/README.md` + `heroes-research-merged.json`）：
  - 已完成 **34 位**：h653 将门有将 / h495 二夫之勇 / h788 雪奋短兵 / h815 蛮王御众（武将 29~32，已在 main）
    ...
    + 上一会话 17 位（45~61）：h800 守静却敌 / h691 持刀从武（上架）/ h604 计谕废立 / h497 中宫追玺（上架）/
    ...
    + 本会话 7 位（63~69）：h102003 方阵掩杀（**上架**）/ h443 迟智难酬 / h684 鏖兵卫主 / h812 锦车持节 /
    h808 自擅江表 / h692 尽言直谏（**上架**，2026-09-20 修正）/ h480 酒池肉林；
  - **剩余 4 位**（口径见重生成的 `docs/下架武将清单.md`；**「机制可补 / 官方无数值」两类卡点均已清零**，
    继续推进只剩 3 位待用户口径 + 1 位按指示挂起）：
    ① **用户指示挂起**：h683 曹彰 五兵之烈（等宝物系统上线再做）；
    ② **待用户给数值/口径**：h2 乱政（负面池动摇/恐慌/燃烧/妖术 4 类伤害率官方无数值）、
       h33 遗志（「恢复极大量兵力」官方无数值）、SP卢植 h102004 中郎尽瘁（「自身进行攻击造成的伤害大幅降低」
       +「援护友军全体」官方均无数值/比例）。
- **本批（33~37）新增引擎件**：
...
- **本会话（45~61）新增引擎件**（均为最小侵入、缺省路径零回归；每条附示例战法）：
  ...
  - `CommandSkill.imperialDecree`（**天子诏令**：① 每回合开始随机点名一名敌军单体 →「受到所有伤害提升」按份叠加
    （`triggerImperialDecrees`，combat.ts 回合开始接入）；② 本侧单位**本回合首次**主动战法伤害/首次普攻
    按 `forceTargetRate` 判定（士气修正 + `skill_trigger` 事件）命中则**强制选中点名目标、无视距离**
    （`decreeForceTarget` 覆盖 dealAttack / 物理伤害段 / 策略伤害段的目标池，每「单位×回合」一次、败亦消耗，
    追击/被动/指挥与代打段不消耗）；③ 回合内点名目标累计受到 `threshold` 次伤害（实际扣兵计 1 次）→
    追加一层受伤提升 + 全属性下降（`decreePunish`，applyDamage 内））——`bb5a787`。
- **本会话（63~69）新增引擎件**（均为最小侵入、缺省路径零回归；每条附示例战法 + 提交号）：
  - **次数型 charges 支持 `[min,max]`**（施加时 `ctx.rng.intInclusive` 取一次；迟智难酬「下 1-2 次策略伤害大幅降低」）——`b94a0b3`；
  - `CommandSkill.allyTroopThreshold`（**友军指定站位兵力首次跌破档位**：`watchPosition` + `thresholds` +
    `casterPositions`，每档每携带者 1 次、同次跨档只结算 1 次；鏖兵卫主「大营跌破 90%/70%/50%」）
    + **属性类 `defenseScaled`**（4 种属性 buff 新增受防御缩放接线）——`7b02ae8`；
  - `CommandSkill.roundStartChance`（**每回合开始前几率判定**：士气修正 + 逐回合 `skill_trigger`，
    命中执行 `output` 并登记 → 回合末 `triggerRoundEndChanceOutputs` 执行 `roundEndOutput`；锦车持节）——`a1fb4ad`；
  - 伤害段**段级 `targetSide`**（physical/strategy_damage 可指定阵营重选池；自擅江表「对友军群体发动攻击」）
    + 段级 `ratePerCast`（只让「此策略攻击」段每次发动递增；含 `hasRatePerCastSegment` 计数修复）——`3202264`；
  - `CommandSkill.allySlotBoost`（**友军主动战法发动率/伤害增益**，随机 N 名其他友军、窗口监视器
    `ctx.allySlotBoostWatch`/`allySlotBoostFired` 升级数量；尽言直谏）+ **`expireAfterOwnAct` 扩到 damage_boost**
    （**目标本次行动结束时清除**，非「下次行动前递减」——用户 2026-09-20 口径）——`963d235`；
  - `healOnDamage` 扩展 `selfOnly`（只对携带者自身的伤害生效）+ `startRound`（第 N 回合起；酒池肉林吸血段）——`e902ed5`。
- **坑（本批踩过）**：`damage_boost.maxStacks` 必须**同时给 `stacks: 1`**（层计数初值），只给 `stack: true` 时
  计数不递增、封顶失效（破阵强袭第 7 次仍 +5%）——文德椒房同款写法。
- **坑（武将 62 补）**：① `CreateStatus` 的 `stack` / `strategyScaled` / `percent`… 是**施加期**字段，
  `pushStatus` 只把最终数值写进 `Status` 实例 → 测试断言只能查 `status.rate/amount`（叠层是否生效看数值变化），
  不要断言 `status.stack`；② 准备主动/指挥的 `targetMode` **不能写 `'single'`**（`listing.test.ts` 明令禁止，
  `'single'` = 最近优先的旧模式），「敌军/友军单体」一律用 `'random_single'`；③ 直接构造 `UnitState` 时
  `general` 是**只读**属性，测试要在 `dummy(..., { activeSkillIds: [...] })` 构造期挂槽，不要事后改写。
- **每将流程**（沿用）：`skills.ts` 定义 → `build_heroes_seed.mjs` 挂槽 → 数据链三条命令  （`build_heroes_seed` / `gen_skill_data` / `sync_hero_mainskill`，性别表变更再加 `sync_hero_meta`）
  ...
  **上架将**（无受属性段且无数值缺口）还须同步：`tests/offline_pool.test.ts` 的 `HEROES.length`（现 88）+1、
...
## 会话交接（分兵阵营 bug 修复 + 胜率统计口径 · 2026-09-26）

### 用户报告

「战报底栏『统计胜率』：只把红蓝两队对调（拖动换方，配置一字未改），两次胜率差得离谱（34.5% / 83.0%）——真的跑了 200 场吗？」

结论：**200 场确实跑了**（面板数字自洽：69+131=200、66+130+3+1=200；0.2~0.4s 与实测一致；同战报重复点结果完全相同），
但**引擎有真 bug** —— 同样两队交换红蓝后结果不互补（用户两队实测 92.5% + 31.5% = 124%）。

### 根因：`executeSplitAttack` 拿固定阵营比「受击目标同队」

```ts
// 修复前（src/engine/action.ts executeSplitAttack）：
const targetTeam = primaryTarget.side === 'my' ? allies : enemies;   // ✗
// 修复后：
const targetTeam = primaryTarget.side === unit.side ? allies : enemies; // ✓
```
`allies` / `enemies` 是**施法者视角**的数组，判据却写死 `'my'`：施法者在 my 侧时恰好等价（正确），
施法者在 enemy 侧（蓝方）时 `targetTeam = allies` ⇒ **蓝方分兵溅射到自己队友身上**（自己打自己）。
实测（司马师/司马昭/文鸯 vs 祝融夫人/华雄/曹操，200 场、同一批种子）：

| | A 当红 | B 当红 | 和 | 逐种子镜像 |
|---|---|---|---|---|
| 修复前 | 92.5% | 31.5% | **124.0%** | 135/200 |
| 修复后 | 68.5% | 31.5% | **100.0%** | 200/200 |

- 回归测试：`tests/split_basic_only.test.ts` 新增「蓝方单位触发分兵必须打受击目标同队、不得溅射自己队友」
  （已双向验证：把那行回退 → 用例失败）。
- **golden 未变**（T1~T10 固定测试集里没有蓝方分兵），全量测试 202 files / **2102 passed**。

### 残留：同速 tie-break 的红侧偏向（引擎层，本次未改）

`buildTurnOrder` / `buildPriorityOrder` 用 `[...myTeam, ...enemyTeam]` + 稳定排序，**同速同站位时红方先行动**。
镜像队（同阵容同加点）实测 A 当红 53.0% + B 当红 53.0% = **106.0%**；同英雄不同加点（速度全同）**103.0%**；
不同阵容（无同速）精确 100.0%。→ 若日后要彻底消除，需在引擎层给同速 tie-break 加随机/中立规则（属口径级改动，动 golden）。

### 统计层修法：胜率面板改成「每颗种子正/反各跑一场」（L4 口径）

`web/winRate.ts`：`simulateWinRate(Async)` 新增 `swapSides`（缺省 true）——
第 k 颗种子跑 2 场（一场我方在左、一场我方在右，结果还原成我方视角），于是：

- 「把红蓝两队对调后再统计」两次结果**必然互补（胜率之和 = 100%）**，与 L4「交换场地」同一口径；
- 同时把上面那条 tie-break 偏向从统计里消掉；
