...
- **工作树/分支**：`.dsh/worktrees/79a0e3b64a07/战斗系统`（上一会话为 `.dsh/worktrees/c00d4f51acc5/战斗系统`，工作区换新但
  起点同 `main` `19fb1a7`）；武将 33~62 全部逐个提交在**未命名分支的 detached HEAD** 上（**未 push、未合并**；落地方式待用户定，
  合并前先看 `docs/工作树落地流程.md`，且合并回主仓库后必须重灌主库——见下）。
  提交一律**显式列路径**（`git add src/... tests/... web/data/...`），不要 `git add -A` / `git add web`。
- **已验证基线**：`npx tsc --noEmit` clean；`npm test` **174 files / 1697 passed**（武将 62 后）；golden 字节一致；
  已实现主战法 **150**、上架池 **88**、《下架武将清单》重生成口径 **待实现 11 / 卡成长率下架 62 / 上架 88**。
...
- **§1.4「需先调研」45 位进度**（总表 `docs/research/README.md` + `heroes-research-merged.json`）：
  - 已完成 **34 位**：h653 将门有将 / h495 二夫之勇 / h788 雪奋短兵 / h815 蛮王御众（武将 29~32，已在 main）
    ...
    + **本会话 1 位（62）**：h795 天子诏令（`bb5a787`，因两处受谋略成长率未确认暂下架）；
  - **剩余 11 位**（口径见重生成的 `docs/下架武将清单.md`，两类卡点 —— **「机制可补、尚未做」已清零**，
    即策略 A 下所有「可直接实现」的武将已全部做完，继续推进必须先拿用户口径）：
    ① **需用户拍板**：h808 自擅江表（官方「对友军群体发动一次攻击」疑为笔误，需确认）、
       h683 五兵之烈（五兵分支互斥需宝物系统）、h692 尽言直谏（作用对象是「主动战法槽」，引擎无槽位机制）；
    ② **官方无数值（策略 A ④：不实现，待用户给口径）**：h2 乱政（负面池 4 类伤害率官方无数值）/ h33 遗志（「恢复极大量兵力」无数值）/
       h684 鏖兵卫主（「下 2 次受到伤害大幅降低」无数字）/ h812 锦车持节（「大幅降低」无数值）/
       h480 酒池肉林（目标行与描述矛盾 + 减伤无数字）/ h443 迟智难酬（减伤无数字）/ SP卢植 中郎尽瘁（伤害降低与援护倍率无数值）/
       SP太史慈 方阵掩杀（「下一次攻击伤害降低」无百分比 + 机制缺失）。
- **本批（33~37）新增引擎件**：
