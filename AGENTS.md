...
### 冲突规则（`action.ts` 的 `inflictStatus`）

先判战法类型（被动/指挥/主动/追击），再判非「伤害」标签是否冲突：
- 同一战法重复触发 → 数值累加 / 控制刷新剩余
- 同类型不同战法 → **控制**：先施加者生效，后施加者被拒（`status_conflict` 事件）；**增益**：数值取较高替换
- 不同类型 → 各自独立计数共存
...
- **已验证基线**：`npx tsc --noEmit` clean；`npm test` **109 files / 1232 passed**；golden 字节一致。
- **下一步（§1.2 剩余 6 个大机制，建议顺序）**：
  1. `连环计`（王允·战法链：依次发动伐谋/迷阵/落雷）——最可控（链式发动已有 `chain` 字段先例）
  2. `赐剑长驱`（友军主动战法首次发动后再发动 40%）——需「友军主动战法再发动」钩子
  3. `鸾凤和鸣`（小乔·控制效果额外 +1 目标）——需「控制目标 +1」维
  4. `率尔方雅`（胡芳·敌我同池随机 3 + 按侧分支）——友军段可复用 `recipientDamageByHigherStat`
  5. `僭号天子`（袁术·伤害转移/结转）——大改，建议放后
  6. `伏波扬砂`（马腾·「伤害共计提升幅度每达到 40%」）——**表述含糊，动手前必须先问用户**
