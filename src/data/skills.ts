...
export const SKILL_REGISTRY: Record<string, Skill> = {
   ...
   * 距离 5，敌军单体，发动率 --。
   * 满级「每回合开始时，随机选择一名敌军单体，使其受到所有伤害提升 16.0%（受谋略属性影响），可叠加，
   *   持续至战斗结束，同时使友军全体本回合内主动战法的首次伤害或首次普通攻击，有 100.0% 几率选中该目标，
   *   无视距离；若回合结束前该敌军单体受到 3 次或以上伤害，则使其受到伤害提升效果额外叠加一次，
   *   并使其全属性下降 12.0（受谋略属性影响），可叠加，持续至战斗结束」；
   * 1 级：受伤提升 8.0% / 几率 50.0% / 全属性下降 6.0。
   * 官方：scripts/skill_extra.json id 200270（指挥 / S / 距离 5 / 骑；effect 受到攻击伤害提高;受到策略攻击伤害提高;
   *   防御属性降低;攻击属性降低;谋略属性降低;速度属性降低）。来源 https://stzb.163.com/m/skilllist/200270.html
   *
   * 口径（策略 A，2026-09-20；推定处已标注）：
   *   ① 官方 desc / desc(level1) 均为「两版本拼接」（前半写「受到 3 次或以上伤害」、后半写「受到 2 次或以上伤害」，
   *      其余文字完全相同；两版首尾相连）→ 按仓库清洗口径取**前半 = 3 次**（17173 2025-01 攻略亦写「3 次以上」，
   *      方向一致）；
   *   ② 「全属性下降 12.0」官方**未写 %**（1 级 6.0）；同句式战法「举义诛暴」官方写「全属性下降 5.0%（受攻击属性影响）」
   *      → 按**百分比**解读（四维各 −12%，1 级 −6%），**推定**；
   *   ③ 两处「受谋略属性影响」官方均未给成长系数 → 基值不缩放（`strategyScaled` 标记在、`growthRate` 留空）→
   *      登记 OFFLINE_MAIN_SKILLS（XP献帝下架，待 derive_growth_rate.mjs 反解）；
   *   ④ 「每回合开始时随机选择一名敌军单体」→ 一类指挥 prep 注册（`triggerImperialDecrees` 每回合开始时点名，
   *      从全部存活敌军随机；距 5 与 3 人队内任意敌军均可达，与按距离筛选等价）；
   *   ⑤ 「友军全体本回合内主动战法的首次伤害或首次普通攻击」= 每「单位 × 回合」**首次**打出伤害时判定一次
   *      （首次伤害与首次普攻二者先到者；`ctx.decreeConsumed` 记账，败亦消耗），命中即强制以点名目标为选靶、
   *      无视距离（`decreeForceTarget` 覆盖普攻 / 主动战法物理段 / 主动战法策略段的目标池；追击、被动、指挥
   *      与代打段不消耗该判定，**推定**）；「友军全体」含施法者自身（仓库 `targetSide:'ally'` 同口径）；
   *   ⑥ 「受到 3 次或以上伤害」= 该回合内点名目标累计**实际扣兵**结算 3 次（含 DoT 跳伤、多段战法逐次计），
   *      恰达阈值那一次触发追加（每回合至多 1 次）→ `decreePunish`；追加的受伤提升与全属性下降均按份叠加、
   *      持续至战斗结束（duration 999）。
   */
  tianzi_zhaoling: {
    id: 'tianzi_zhaoling',
    name: '天子诏令',
    ...
    targetMode: 'random_single',
    targetSide: 'enemy',
    tags: ['damage_boost', 'debuff_attack', 'debuff_defense', 'debuff_strategy', 'debuff_speed'],
    // 本战法全部效果由 imperialDecree 逐回合驱动（回合开始点名 + 首击强制选靶 + 回合内受击追加）
    output: [],
    imperialDecree: {
      // ① 每回合点名：受到所有伤害 +16%（1 级 8%；受谋略未确认 → 基值），可叠加、持续至战斗结束
      takenBoostRate: 16,
      strategyScaled: true,
      // ⑤ 本回合首次主动伤害 / 首次普攻强制选中点名目标的几率（满级 100%、1 级 50%，走士气修正）
      forceTargetRate: 1,
      // ⑥ 回合内受到 3 次或以上伤害 → 追加一层受伤提升 + 全属性下降
      threshold: 3,
      // ② 追加时四维各 −12%（1 级 −6%；受谋略未确认 → 基值），可叠加、持续至战斗结束
   ...
   *
   * 口径（策略 A，2026-09-20；推定处已标注）：
   *   ① 追击主段沿用**本战法整体目标 `[攻击目标]`**，不受距离约束（官方有效距离栏「--」）→ `range: 0`（扬威先例）；
   *   ② 「使其进行下一次攻击的伤害大幅度降低」——用户 2026-09-20 口径：凡描述写「大幅度」的均为**极大值**，
   *      即 rate = **−99.99（−9999%）**，配合 `buffMult` 的 10% 伤害下限使该次攻击伤害压到最低，
   *      战报显示为「造成的伤害大幅降低」（不显示 9999%）；
   *   ③ 「下一次攻击」= `charges: 1` 的次数型造成侧减伤（美人计「下一次攻击或策略攻击造成伤害降低」先例：
   *      下一次造成伤害即消耗，不加伤害来源过滤），**推定**；
   *   ④ 无「受属性影响」段、无数值缺口（235% 与「大幅度=极大值」均已确定）→ **上架**（不登记 OFFLINE_MAIN_SKILLS）。
   */
  fangzhen_yansha: {
    id: 'fangzhen_yansha',
    name: '方阵掩杀',
    type: 'pursuit',
    range: 0, // 官方「--」（追击战法不适用）：追击主段沿用普攻目标，不受距离约束
    triggerRate: 0.4, // 官方 40%
    tags: ['damage', 'damage_boost'],
    output: [
      // ① 对攻击目标再次发动猛攻 235%
      { kind: 'physical_damage', rate: 235 },
      // ② 使其「下一次攻击」造成的伤害大幅降低（极大值 −9999%，下一次造成伤害后消耗）
      {
        kind: 'inflict_status',
