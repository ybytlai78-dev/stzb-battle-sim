...
本文件是 **199 个可学习通用战法中 skipped 战法**的机制清点（当前 **47** 个，含援军之策：机制已有、仅标 `pending_strategy_scale`）。每个战法因需要引擎缺失机制而暂缓录入（未实现不算遗漏，算**待机制**）。受谋略缩放未确认的，只打标签、本轮不录入。
...
- **距离 +1 前四张 + 特殊目标选择全部 6 张已入库**（2026-09-19 p4，10 个）：远攻秘策、远攻之策、远攻奇略、远攻强化（`range_buff` 攻击距离，机制已有）；近攻（`single` 模式 = 有效距离内最近，本批首次启用）、远射 / 连环段1、兼弱攻昧、始计、铁戟金戈。新增引擎能力：`farthest` 目标模式；伤害段 `targetPick:'lowest_defense_in_range' / 'lowest_strategy_in_range'`（`unitsInSkillRange` + 生效属性比较）；`inflict_status.targetPick:'highest_troops_enemy'`；铁戟金戈用 `random_pick count:1` 实现「每次释放 50/50 抽 2 目标 330% / 3 目标 225%」。受谋略缩放段（连环 132/198、兼弱攻昧 159、始计 20%/30%）`growthRate` 留空，已登记《成长率待补名单》§三。合纵连横（4 项新机制）后置。

**确认仍缺**：士气比较 / 士气降低 / 援护 / 受击触发（垒实迎击、百战无怯、疾风迅雷等仍缺） / 战法距离+1（合纵连横） / 兵力比例 / 恢复次数递增（胜敌益强） / 下一次攻击增减伤（仅闪击） / 追击多段 / 攻城属性 / 移除敌军有益 / 女武将组合 / 准备跳过 等。反击已作为 `counter` 状态入库（反击之策）；诱敌深入仍 skipped。
...
2. **士气比较/降低**（5）— `moraleRate` 相邻扩展（望风而降 / 激水之疾 / 蓄盈待竭 / 胜负先征 / 及锋而试）
3. **移除敌军有益**（4：看破 / 索敌 / 驱逐 / 火积）＋**援护**（3：援护 / 移花接木 / 一夫当关）
4. **受谋略缩放批次** — 援军之策（`pending_strategy_scale`，机制已有只差成长率）及后续同类
5. **战法距离+1 + 阵营条件**（合纵连横：含对非自身阵营增伤、普攻后围困）
6. 其余单机制战法按需推进（先手类 / 攻城属性 / 兵力比例 / 准备跳过 / 恢复次数递增 / 追击多段 / 下一次伤害结算 等）
