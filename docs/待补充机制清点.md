...
本文件是 **199 个可学习通用战法中 skipped 战法**的机制清点（当前 **37** 个，含援军之策：机制已有、仅标 `pending_strategy_scale`）。每个战法因需要引擎缺失机制而暂缓录入（未实现不算遗漏，算**待机制**）。受谋略缩放未确认的，只打标签、本轮不录入。
...
**确认仍缺**：战法距离+1（合纵连横） / 兵力比例 / 恢复次数递增（胜敌益强） / 下一次攻击增减伤（仅闪击） / 追击多段 / 攻城属性 / 移除敌军有益 / 女武将组合 / 准备跳过 等。已解决：反击 / 诱敌深入（p6，`counter` 状态 + `onHurt.applyTo:'source'`）；受击触发（p6，`onHurt` 数组 + `stacksReduceHeal` + `onBasicHit`）；**援护机制已在 p6 落地**（`cover` 状态 + 段级 `casterPositions`，垒实迎击），援护组（援护 / 移花接木 / 一夫当关）仅剩数据录入；士气比较（`morale_branch.compareTo:'caster'`）与士气降低（`morale_boost` 负值）已于 p5 入库。
...
1. **移除敌军有益**（4：看破 / 索敌 / 驱逐 / 火积）＋**援护组**（3：援护 / 移花接木 / 一夫当关，`cover` 机制已就绪）
2. **受谋略缩放批次** — 援军之策（`pending_strategy_scale`，机制已有只差成长率）及后续同类
3. **战法距离+1 + 阵营条件**（合纵连横：含对非自身阵营增伤、普攻后围困）
4. 其余单机制战法按需推进（先手类 / 攻城属性 / 兵力比例 / 准备跳过 / 恢复次数递增 / 追击多段 / 下一次伤害结算 等）
