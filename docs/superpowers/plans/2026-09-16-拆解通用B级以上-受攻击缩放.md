...
**Goal:** 补 `attackScaled`、taken charges 受击消耗、`decayFifths`、被动物理叠层与策略输出级 `range`，完整入库万箭齐发 / 文伐 / 不攻 / 恃强淬锋。

**Architecture:** 只加可选字段，不按 `skill.id` 特判、不抽新战法类型。增减伤受攻击在施加时按 `speedScaled` 同路径缩放并写入 `rate`；文伐 taken 在 `applyDamage` 实际扣兵后再扣 charges（当次仍计入）；恃强五份在受策略扣兵后衰减；物理叠层走 `PassiveSkill.selfPhysBoost`（回合开始走 `tickRoundStartStatuses`，**不要**走被动 `roundStartRepeat`）。
...
| `src/engine/action.ts` | 受攻击缩放；taken 受击消耗；caused-only 攻击 charges；五份衰减；物理叠层；输出级 range |
