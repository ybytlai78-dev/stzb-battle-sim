...
`consumeAttackCharges` 增加 `hit?: DamageHitContext`，循环里 `if (!statusMatchesHit(s, hit)) continue;`。所有调用点传入对应 hit（普攻 `{ damageSource:'basic', damageType:'physical' }`；物理战法 `{ damageSource:'skill', damageType:'physical', skillType }`；策略战法 `{ damageSource:'skill', damageType:'strategy', skillType }`）。无上下文的旧调用保持不传（`statusMatchesHit` 无 hit 时返回 true，兼容青丘媚祸等无过滤 charges）。
