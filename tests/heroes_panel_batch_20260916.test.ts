...
describe('批量补入 · XP 卡（season=XP，100xxx 段）', () => {
  ...
  it('XP 卡一律空槽下架（主战法未实现）', () => {
    ...
    // h800（XP蒋琬·守静却敌）已实现（因三处「受谋略」成长率未确认暂下架，2026-09-20 武将 45）→ 从本表移出
    // h691（XP周仓·持刀从武）已实现并上架（2026-09-20 武将 46）→ 从本表移出
    // h675（XP刘表·抚民励德）已实现（因减伤受谋略成长率未确认暂下架，2026-09-20 武将 49）→ 从本表移出
    // h791（XP陆逊·疲兵沮意）已实现（因避锐受谋略成长率未确认暂下架，2026-09-20 武将 50）→ 从本表移出
    // h787（XP程昱·审时定计）已实现（因两处受谋略成长率未确认暂下架，2026-09-20 武将 57）→ 从本表移出
    // h784（XP周瑜·衔命建功）已实现（因策略攻击受谋略成长率未确认暂下架，2026-09-20 武将 59）→ 从本表移出
    // h652（XP王朗·言出必克）已实现（因策略减伤受谋略成长率未确认暂下架，2026-09-20 武将 60）→ 从本表移出
    for (const id of ['h684', 'h792', 'h795', 'h808']) {
  ...
  it('XP丁奉 h788：主战法雪奋短兵已实现（动摇成长未确认 → 暂下架，不再空槽）', () => {
    const r = HERO_RECORDS['h788'];
    expect(r.name).toBe('XP丁奉');
    expect(r.mainSkillName).toBe('雪奋短兵');
    expect(r.mainSkillId).toBe('xuefen_duanbing');
    expect(isHeroListed(r)).toBe(false);
  });

  it('XP孟获 h815：主战法蛮王御众已实现（减伤受防御成长未确认 → 暂下架，不再空槽）', () => {
    const r = HERO_RECORDS['h815'];
    expect(r.name).toBe('XP孟获');
    expect(r.mainSkillName).toBe('蛮王御众');
    expect(r.mainSkillId).toBe('manwang_yuzhong');
    expect(isHeroListed(r)).toBe(false);
  });

  it('XP左慈 h802：主战法奇门遁甲已实现 → 上架（不再空槽）', () => {
    const r = HERO_RECORDS['h802'];
