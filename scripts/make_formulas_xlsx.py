...
ws["A2"] = "攻击伤害 / 谋略伤害 / 兵力恢复 计算公式 + 因变量-自变量图表"
...
row += 1
shared = [
    "单位伤害曲线（攻击/谋略/恢复共用）：unitDamage = 300×兵/(3500+兵)，9000 兵 ≈216、10000 兵 ≈222",
    ...
    "增减伤因子（单一总和，攻击/谋略统一，v0.14 权威复核）：因子 = max(10%, 1 + Σ增伤 − Σ减伤)；例 增60%+减60% → 1.0",
...
labels_in = [
    ...
    (6, "伤害率（%）", 100, "0", "攻击伤害战法伤害率，通常不受属性影响"),
...
ws["C11"].comment = Comment("单一总和模型：因子 = max(10%, 1 + Σ增伤 − Σ减伤)（攻击/谋略统一，v0.14 权威复核）", "公式表")
...
add_chart(ws, "G14", "攻击伤害 vs 兵力（三部分拆解）", "兵力", "伤害",
...
anchors2 = [
    ("攻击兵力基础伤害（10000 兵，期望≈211）", "=ROUND(373*10000/(7700+10000),0)", 211),
